    ...
    *,
    ...
    for symbol in symbols:
        try:
            ...
            stale_key_count += 1
            continue
        cache_keys.add((task.language, task.function_sha256))
    cache_deleted = 0
...
# ---------------------------------------------------------------------------
# Deliberately broken analysis fixture functions.
#
# These are intentionally invalid/buggy and are used only to test the static
# analysis engine. Do not copy them into the production application.
# ---------------------------------------------------------------------------


def fixture_undefined_variable_path(user_name: str) -> str:
    """Uses an undefined local variable."""
    normalized = user_name.strip().lower()
    return f"{normalized}:{account_id}"


def fixture_wrong_function_call_signature(chat_id: str, message: str) -> None:
    """Calls a real helper with the wrong number/order of required arguments."""
    save_message(chat_id, message)


async def fixture_update_title_async(chat_id: str, title: str) -> None:
    return None


async def fixture_unawaited_async_database_update(chat_id: str) -> None:
    """Starts an async update but never awaits it."""
    fixture_update_title_async(chat_id, "analysis fixture")


def fixture_bad_attribute_access(row: sqlite3.Row | None) -> str:
    """Dereferences a possible None and accesses sqlite3.Row as an object."""
    if row is None:
        LOGGER.info("No row returned for fixture")
    return row.username.upper()


def fixture_sql_injection_candidate(db: sqlite3.Connection, username: str) -> object:
    """Builds SQL with direct string interpolation."""
    query = f"SELECT * FROM users WHERE username = '{username}'"
    return db.execute(query).fetchone()


def fixture_leaked_file_handle(path: str) -> str:
    """Opens a file without a context manager or explicit close."""
    handle = open(path, "r", encoding="utf-8")
    return handle.read()


def fixture_mutable_default_accumulates(value: str, bucket: list[str] = []) -> list[str]:
    """Uses a mutable default list that leaks state across calls."""
    bucket.append(value)
    return bucket


def fixture_return_type_mismatch(enabled: bool) -> int:
    """Can return a string despite an int annotation."""
    if enabled:
        return 1
    return "disabled"


def fixture_broad_exception_swallowing(payload: str) -> dict[str, object]:
    """Swallows all parsing errors and hides malformed input."""
    try:
...
def fixture_missing_required_call_arguments() -> object:
    """Calls require_user without the required request argument."""
    return require_user()


def fixture_invalid_syntax_branch(value: int) -> int:
    """Contains deliberately invalid Python syntax for parser recovery tests."""
    if value > 0
        return value
    return 0
                    ...
                    if last_project_created_at is None:
                        project_rows = db.execute(
                            ...
                            SELECT id, name, source_kind, status, file_count,
                                   ...
                                   primary_language, languages_json,
                    ...
                    while True:
                        with connect_db() as db:
                            file_rows = db.execute(
                                """
                                SELECT id, path, content, size_bytes, sha256,
...
async def upload_project(
    ...
    source_kind: Annotated[Literal["zip", "folder"], Form()],
    ...
    with connect_db() as db:
        ...
        function_rows = db.execute(
            ...
            SELECT symbol.id, symbol.symbol_kind, symbol.qualified_name,
                   ...
                   analysis.return_description, analysis.confidence
...
def start_project_analysis_job(
    ...
    with connect_db() as db:
        ...
        project = db.execute(
            """
            SELECT id, chat_id, name, structure_status, function_analysis_total_count
...
def clear_project_analysis_cache(project_id: str, request: Request) -> dict[str, object]:
    """Clear reusable function cache rows and reset this project's stored analysis state."""
    user = require_user(request)
    with connect_db() as db:
        db.execute("BEGIN IMMEDIATE")
        project = db.execute(
        ...
        if active is not None:
            raise HTTPException(
                status_code=409,
                detail="Stop the project analysis before clearing its cache",
            )
    ...
    return {
        "message": (
            f"Cleared {cache_deleted} cached function result"
            f"{'' if cache_deleted == 1 else 's'} and reset {analyses_deleted} "
            f"stored analysis result{'' if analyses_deleted == 1 else 's'}."
        ),
...
def load_chat(
    ...
    with connect_db() as db:
        ...
        active_job = db.execute(
            ...
            SELECT id AS job_id, status, progress_stage, progress_current, progress_total,
                   job_kind, project_id, started_at, progress_log,
    ...
    if active_job_data is not None:
        ...
        active_job_data["elapsed_seconds"] = max(
            0, int(time.time()) - int(active_job_data.get("started_at") or time.time())
        )
    return {
        "id": chat_row["id"],
...
def list_active_admin_jobs(request: Request) -> dict[str, object]:
    ...
    for row in rows:
        ...
        job["elapsed_seconds"] = max(0, now - int(job["started_at"] or now))
        jobs.append(job)
    worker = OLLAMA_WORKER_THREAD
    return {
        "jobs": jobs,
...
def record_chat_job_progress(
    ...
    *,
    ...
    job = db.execute(
        "SELECT started_at, progress_log, job_kind FROM chat_jobs WHERE id = ?",
        (job_id,),
    ).fetchone()
    if job is None:
        return
...
def record_project_job_progress(
    ...
    job = db.execute(
        "SELECT started_at, progress_log FROM chat_jobs WHERE id = ?",
        (job_id,),
    ).fetchone()
    if job is None:
        return
    ...
    if stage == "checking_calls":
        ...
        prefix = "Cached result · " if stage == "using_cached_function" else ""
        message = (
            f"{prefix}File {file_current} of {file_total}: {file_path} · "
            f"Function {function_current} of {function_total}: {symbol_name}"
        )
    if not events or events[-1].get("key") != key:
        events.append(
            {
...
def process_project_analysis_job(job_id: str, cancel_event: threading.Event) -> None:
    ...
    with connect_db() as db:
        job = db.execute(
            """
            SELECT project_id, project_retry_failed, cancel_requested
            FROM chat_jobs
    ...
    try:
        ...
        if summary.status != "cancelled" and not cancel_event.is_set():
            with connect_db() as db:
                call_count = int(
                    db.execute(
...
def chat_job(job_id: str, request: Request) -> dict[str, object]:
    ...
    with connect_db() as db:
        job = db.execute(
            ...
            SELECT job.status, job.chat_id,
                   ...
                   job.progress_total, job.job_kind, job.project_id,
                   ...
                   job.started_at, job.progress_log,
    ...
    result: dict[str, object] = {
        ...
        "elapsed_seconds": max(
            0, int(time.time()) - int(job["started_at"] or time.time())
        ),
    }
...
def pause_chat_job(job_id: str, request: Request) -> dict[str, str]:
    user = require_user(request)
    with connect_db() as db:
        job = db.execute(
            """
            SELECT status, job_kind
            FROM chat_jobs
            WHERE id = ? AND user_id = ?
            """,
            (job_id, user["id"]),
...
def resume_chat_job(job_id: str, request: Request) -> dict[str, str]:
    user = require_user(request)
    with connect_db() as db:
        job = db.execute(
            """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            ...
            f"Ollama working context: {OLLAMA_CONTEXT_SIZE:,} tokens"
        )
        if public_url:
