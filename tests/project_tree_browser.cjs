...
const server = http.createServer(async (req,res) => {
  ...
  if(req.url==='/api/projects/qa-project/functions/101/callers')data={function:functions[0],caller_count:3,callers:[{id:1,caller_symbol_id:102,caller_name:'request_function_analysis',path:'src/main.py',start_line:54,usage_kind:'assignment',callee:'build_verified_source_inventory',code:'build_verified_source_inventory(source)'},{id:2,caller_symbol_id:null,caller_name:null,path:'src/main.py',start_line:70,usage_kind:'statement',callee:'build_verified_source_inventory',code:'build_verified_source_inventory(default_source)'},{id:3,caller_symbol_id:102,caller_name:'request_function_analysis',path:'src/main.py',start_line:58,usage_kind:'assignment',callee:'build_verified_source_inventory',code:'build_verified_source_inventory(fallback_source)'}]};
...
(async()=>{
  ...
  assert.equal(await evaluate("document.querySelector('#function-callers-summary').textContent"),'Functions calling build_verified_source_inventory (2)');
  assert.deepEqual(await evaluate("[...document.querySelectorAll('.function-caller')].map(row=>row.textContent)"),['main.py/request_function_analysis','main.py/Module-level code']);
  assert.equal(await evaluate("document.querySelector('.function-caller-meta,.function-caller-code')===null"),true);
  assert.doesNotMatch(await evaluate("document.querySelector('#function-callers-list').textContent"),/src\/main\.py|build_verified_source_inventory\(source\)/);
