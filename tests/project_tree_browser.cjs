...
const functions=[
  ...
  {id:201,file_id:2,name:'helper_one',qualified_name:'helper_one',start_line:1,end_line:3,header:'def helper_one(value: object) -> object:',return_lines:[{line:2,code:'return value',flow_dependent:true},{line:3,code:'return None',flow_dependent:true}],description:'Checks a source-derived condition and returns the validated object.',description_status:'deterministic'},
...
(async()=>{
  ...
  assert.match(await evaluate("document.querySelector('.tree-function[data-symbol-id=\"101\"]').title"),/^def build_verified_source_inventory.*Builds a verified inventory.*Return \(line 35\): return inventory/s);
  ...
  assert.match(await evaluate("document.querySelector('.tree-function[data-symbol-id=\"201\"]').title"),/^def helper_one.*Static analysis.*Return 1 \(flow dependent · line 2\): return value.*Return 2 \(flow dependent · line 3\): return None/s);
